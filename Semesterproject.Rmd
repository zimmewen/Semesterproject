...
```{r, echo=FALSE, cols.print=4, rows.print=3, fig.height=4, fig.width=5}

wildschwein #reicht aus um als Tabelle anzuzeigen (in html-options oben definiert)

tmap_mode("view")+
tm_shape(wildschwein)+
  tm_dots("TierName", alpha = 0.5)
...
```{r, cols.print=4, rows.print=3, fig.height=4, fig.width=5}
...
schreck #reicht aus um als Tabelle anzuzeigen (in html-options oben definiert)

leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addCircles(data = schreck, lng = ~lon, lat = ~lat) %>%
  setView(lng = 7.08, lat = 46.996, zoom = 12)
...
```{r, fig.keep='last', fig.height=4, fig.width=5}
...
WSS_2015_01_besendert <- wildschwein %>%
 filter(Datum > "2015-04-08" & Datum < "2015-08-12") %>% #Laufzeit plus gleiche Zeitdauer vorher und nachher
...
WSS_2015_03_besendert <- wildschwein %>%
 filter(Datum > "2015-04-12" & Datum < "2015-08-10") %>% #Laufzeit plus gleiche Zeitdauer vorher und nachher
...
WSS_2015_04_besendert <- wildschwein %>%
 filter(Datum > "2015-04-22" & Datum < "2015-08-05") %>% #Laufzeit plus vorher und nachher
...
WSS_2016_01_besendert <- wildschwein %>%
 filter(Datum > "2016-03-16" & Datum < "2016-05-12") %>% #Laufzeit plus gleiche Zeitdauer vorher und nachher
...
WSS_2016_05_besendert <- wildschwein %>%
 filter(Datum > "2016-05-16" & Datum < "2016-08-14") %>% #Laufzeit plus gleiche Zeitdauer vorher und nachher
...
tmap_mode("view")+ 
tm_shape(wildschwein_homerange)+
  tm_polygons("TierName", alpha = 0.5, title ="")
...
# Trajektorien
ggplot()+
  geom_path(wildschwein, mapping=aes(E, N, color = TierName))+
  geom_point(wildschwein, mapping=aes(E, N, color = TierName))+
  coord_sf(datum = 2056)+
  facet_wrap(~TierName)+
  labs(x = "", y = "")+
...
# Achtung: Ausreisser bleiben unberücksichtigt
...
Die WSS_2016_06 und WSS_2015_04 liegen sehr dicht beieinander, ihre Radien überschneiden sich. Da sie jedoch zu unterschiedlichen Zeiten liefen, braucht es in der weiteren Analyse nicht berücksichtigt werden. Der Radius um den WSS_2016_06 erwies sich als leer (keine Wildschweine in unmittelbarer Nähe) innerhalb der Laufzeit, somit wurde dieser in weitere Analysen nicht miteinbezogen.
...
WSS_2015_01_besendert_buffer <- st_intersection(WSS_2015_01_besendert, WSS_2015_01_buffer)
...
# Verschneide zuvor gewählte Wildschweine in Schreck-Laufzeit mit Buffer
WSS_2015_03_besendert_buffer <- st_intersection(WSS_2015_03_besendert, WSS_2015_03_buffer)
...
# Verschneide zuvor gewählte Wildschweine in Schreck-Laufzeit mit Buffer
WSS_2015_04_besendert_buffer <- st_intersection(WSS_2015_04_besendert, WSS_2015_04_buffer)
...
# Verschneide zuvor gewählte Wildschweine in Schreck-Laufzeit mit Buffer
WSS_2016_01_besendert_buffer <- st_intersection(WSS_2016_01_besendert, WSS_2016_01_buffer)
...
# Verschneide zuvor gewählte Wildschweine in Schreck-Laufzeit mit Buffer
WSS_2016_05_besendert_buffer <- st_intersection(WSS_2016_05_besendert, WSS_2016_05_buffer)
...
# Verschneide zuvor gewählte Wildschweine in Schreck-Laufzeit mit Buffer
WSS_2016_06_besendert_buffer <- st_intersection(WSS_2016_06_besendert, WSS_2016_06_buffer)
# leer, weil effektiv keine Tier während Laufzeit im Buffer
# wenn andere Buffergrösse nochmal schauen, ansonsten brauch dieser Schreck nicht weiter verwendet werden

leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addCircles(data = schreck, lng = ~lon, lat = ~lat) %>%
  addCircles(data = schreck_200buffer, lng = ~lon, lat = ~lat, radius = ~200) %>%
  setView(lng = 7.08, lat = 46.996, zoom = 12)
...
WSS_2015_01_besendert <- WSS_2015_01_besendert %>%
  mutate(im_buff = st_intersects(WSS_2015_01_besendert, WSS_2015_01_buffer, sparse = FALSE)[,1])
...
WSS_2015_03_besendert <- WSS_2015_03_besendert %>%
  mutate(im_buff = st_intersects(WSS_2015_03_besendert, WSS_2015_03_buffer, sparse = FALSE)[,1])
...
WSS_2015_04_besendert <- WSS_2015_04_besendert %>%
  mutate(im_buff = st_intersects(WSS_2015_04_besendert, WSS_2015_04_buffer, sparse = FALSE)[,1])
...
WSS_2016_01_besendert <- WSS_2016_01_besendert %>%
  mutate(im_buff = st_intersects(WSS_2016_01_besendert, WSS_2016_01_buffer, sparse = FALSE)[,1])
...
WSS_2016_05_besendert <- WSS_2016_05_besendert %>%
  mutate(im_buff = st_intersects(WSS_2016_05_besendert, WSS_2016_05_buffer, sparse = FALSE)[,1])
...
wildschwein_WSS_all_duration <- bind_rows(wildschwein_WSS_2015_01_duration, wildschwein_WSS_2015_03_duration, wildschwein_WSS_2015_04_duration,wildschwein_WSS_2016_01_duration, wildschwein_WSS_2016_05_duration)
...
Bei der Ermittlung wie viele Wildschweine sich vor, während und nach der Laufzeit innerhalb des gewählten Radius von 200m um einen WSS aufgehalten haben kann keine eindeutige Aussage getroffen werden. In keinem Radius befinden sich während der Laufzeit deutlich weniger Tiere als vorher oder nachher. Bei zwei von fünf WSS (WSS_2016_01 und WSS_2016_05) zeigen immerhin nach der Laufzeit weniger Datenpunkte. Gleichwohl scheint sich bei zwei anderen WSS (WSS_2015_01 und WSS_2015_03) nachher sogar mehr Schwarzwild in der Nähe aufzuhalten. WSS_2015_04 zeigt ein ebenso uneindeutiges Bild. Diese Ambivalänz zeigt sich auch in der nachfolgenden statistischen Analyse.
...
WSS_all_besendert_buffer <- rbind(WSS_2015_01_besendert_buffer, WSS_2015_03_besendert_buffer, WSS_2015_04_besendert_buffer, WSS_2016_01_besendert_buffer, WSS_2016_05_besendert_buffer)


ggplot()+
  geom_sf(schreck_200buffer, mapping = aes())+
  geom_sf(WSS_all_besendert_buffer, mapping = aes(color = TierName, alpha = 0.3))+
  coord_sf(datum=2056)+
  labs(title="xxx",
       color = "")+
  guides(alpha = "none")+
...
Betrachtet man die Aufenthaltsdauer der Tiere innerhalb des Radius sieht man, dass sich das Tier Caroline eine vergleichsweise hohe Anzahl Stunden dort aufgehalten hat. Auf sie scheint der WSS nach seiner Laufzeit nur einen minimalen Effekt zu haben. Dies bestätigt auch die nachfolgende statistische Berechnung. Auch fünf weitere Tiere (Miriam, Ruth, Sabine und Ueli) scheinen während der WSS lief keine veränderte Aufenthaltsdauer aufzuweisen. Für das Tier Ruth steht dies im Gegensatz zur ANOVA-Berechnung, bei der ein Unterschied festgestellt wurde. Laut dieser Grafik scheint der WSS eher auf Rosa einen Effekt zu haben, welche sich zumindest in der Peridode nachher gar nicht näher als 200m aufhält.

```{r, echo=FALSE, fig.height=4, fig.width=5}

plot <- subset(wildschwein_WSS_all_duration, TierName!="Donar" & TierName!="Franz" & TierName!="Gaby" & TierName!="Joanna" & TierName!="Venus")

plot$Period <- factor(plot$Period,ordered = T,levels = c("nachher","während","vorher"))
...
box2 <- ggplot2::ggplot(wildschwein_WSS_2015_03_duration, aes(x = Period, y= dauer)) +
  geom_boxplot(fill = "white", color = "black", size = 1) +
  scale_y_log10() +
  labs(x="", y="log10-Transformation")

(buffer2n / box2)+
  plot_annotation("WSS_2015_03 - Effekt nachweisbar")

summary.lm(aov(log10(wildschwein_WSS_2015_03_duration$dauer)~wildschwein_WSS_2015_03_duration$Period))

tuk2 <- TukeyHSD(aov(log10(wildschwein_WSS_2015_03_duration$dauer)~wildschwein_WSS_2015_03_duration$Period))
...
box1 <- ggplot(wildschwein_WSS_2015_01_duration, aes(x = Period, y= dauer)) +
  geom_boxplot(fill = "white", color = "black", size = 1) +
  scale_y_log10() +
  labs(x="", y="log10-Transformation")

(buffer1n / box1)+
    plot_annotation("WSS_2015_01 - Kein Effekt nachweisbar")
...
box_ruth <- ggplot(Ruth_duration, aes(x = Period, y= dauer)) +
  geom_boxplot(fill = "white", color = "black", size = 1) +
  scale_y_log10() +
  labs(x="", y="log10-Transformation")

(buffer_ruth / box_ruth)+
    plot_annotation("Ruth bei WSS_2015_03 - Effekt nachweisbar")
...
# Boxplot nach Logarithmus-Transformation 
box_caro <- ggplot2::ggplot(Caroline_duration, aes(x = Period, y= dauer)) +
  geom_boxplot(fill = "white", color = "black", size = 1) +
  scale_y_log10() +
  labs(x="", y="Log10-Transformation")

(buffer_caro / box_caro)+
    plot_annotation("Caroline bei WSS_2015_01 - Kein Effekt nachweisbar")
